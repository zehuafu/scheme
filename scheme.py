...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        return scheme_apply(self, operands.map(lambda x: scheme_eval(x, env)), env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if type(expressions.second.first) == int:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        if scheme_symbolp(bindings.first.first):
            x = Pair(bindings.first.first, x)
            y = Pair(scheme_eval(bindings.first.second.first, env), y)
        else:
            raise SchemeError
        bindings = bindings.second
    return env.make_child_frame(x, y)
